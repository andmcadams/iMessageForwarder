...
while True:
	# Handle basic messages and attachments
	cursor = conn.execute('select ROWID, chat_id, text from message')
	rows = cursor.fetchall()
	for row in rows:
		rowId, chatId, text = row
		getToChat(chatId)
		time.sleep(1.5)
		...
		time.sleep(0.2)
		pyautogui.press('enter')
		cursor = conn.execute('delete from message where ROWID = ?', (rowId, ))
		conn.commit()

	# Create new chats
	cursor = conn.execute('select ROWID, recipient_string, text from chat')
	rows = cursor.fetchall()
	for row in rows:
	...
	# Rename group chat
		#TODO: Implement
	# React (slowest)
	cursor = conn.execute('select ROWID, chat_id, associated_guid, associated_type from reaction')
	# Only handle one per round so that messages don't get backed up
	row = cursor.fetchone()
	if row is not None:
		rowId, chatId, assocGUID, assocType = row
