    ...
    * This frame only allows vertical scrolling
    ...
    def __init__(self, parent, minHeight, minWidth, *args, **kw):
        ...
        def _configure_interior(event):
            ...
            size = (max(minWidth, self.interior.winfo_reqwidth()),
                    max(minHeight, self.interior.winfo_reqheight()))
            self.canvas.config(scrollregion="0 0 %s %s" % size)
            if self.interior.winfo_reqwidth() != self.canvas.winfo_width():
                # update the canvas's width to fit the inner frame
                self.canvas.config(width=max(minWidth,
    ...
    def _configure_canvas(self, event):
        ...
        # Add/remove scrollbars depending on whether or not they're necessary
    ...
    def _bound_to_mousewheel(self, event):
        ...
        self.canvas.bind_all('<Button-4>', (lambda event:
                             self._on_mousewheel_linux(-1, event)))
        self.canvas.bind_all('<Button-5>', (lambda event:
                             self._on_mousewheel_linux(1, event)))
